...
class BM_Tasks extends BaseController
{
    ...
    public function addBM_Task()
    {
        ...
        if ($branch > 0) {
            // Check if a record already exists for this date and branch
            $existedRecord = $bmTasksModel->checkExistingRecord($taskDate, $branch);

            log_message('error', 'Received JSON Data: ' . json_encode($existedRecord));

            $mid = $existedRecord ? ($existedRecord['mid'] ?? null) : null;

            if ($existedRecord) {
                return $this->respond(['status' => true, 'message' => 'Record already exists for this date and branch.', 'data' => $mid], 200);
            }
            ...
            if (isset($jsonData->report_amendments) && is_array($jsonData->report_amendments)) {
                foreach ($jsonData->report_amendments as $amendment) {
                    try {
                        $report_amendment = [
                            'task_id' => $insertId, // Use the main task ID
            ...
            return $this->respond([
                'status' => true,
                'message' => 'BM Task added successfully.',
                'data' => $insertId
            ], 201);
        }
    ...
    public function getBM_TaskListForAdmin()
    {
        ...
        // Pass the parameters to the model
        $mtDetails = $bmTasksModel->getBM_TaskListForAdmin($role, $user, $selectedMonth, $selectedBranch, $selectedCluster, $selectedZone, $selectedDate);
