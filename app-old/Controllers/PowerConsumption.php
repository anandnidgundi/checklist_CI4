...
class PowerConsumption extends BaseController
{
    ...
    public function getPowerConsumptionAdminList($month)
    {
        ...
        $selectedMonth = $this->request->getPost('month') ?? ($jsonData['month'] ?? $this->request->getVar('month')) ?? '2025-04';
        $selectedDate = $this->request->getPost('selectedDate') ?? ($jsonData['selectedDate'] ?? $this->request->getVar('selectedDate'));


        $powerConsumptionList = $powerConsumptionModel->getPowerConsumptionAdminList($role, $emp_code, $zone_id, $selectedCluster, $selectedBranch, $selectedMonth, $selectedDate);
    ...
    public function editPowerConsumption($id = null)
    {
        ...
        $cluster_id = $branchDetails['cluster_id'];
        $zone_id = $branchDetails['zone_id'];
