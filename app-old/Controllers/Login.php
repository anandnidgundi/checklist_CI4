...
class Login extends BaseController {
    ...
    public function index() {
       ...
       // log_message('error', 'User Password '. $user->password);
       // log_message('error', 'Decrypted Password '. $password);
        //log encrypted password
        // log_message('error', 'Encrypted Password '. md5($password));
     ...
     //   log_message('error', 'Payload '. json_encode($payload));
        ...
        if ( !$token ) {
            return $this->respond( [ 'error' => 'Failed to generate JWT.' ], 500 );}else{
                $data = [
