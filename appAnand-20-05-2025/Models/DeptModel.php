...
class DeptModel extends Model
{
    ...
    public function getUserBranches($branches)
    {
        try {
            ...
            $db2 = \Config\Database::connect('secondary');
            if (!$db2) {
                log_message('error', 'Failed to connect to secondary database');
                return [];
            }

            // Convert CSV to array and filter out empty values
            $branchArray = array_filter(array_map('trim', explode(',', $branches)));
            ...
            log_message('error', 'Database error in getUserBranches: ' . $e->getMessage());
            return [];
        }
    }
