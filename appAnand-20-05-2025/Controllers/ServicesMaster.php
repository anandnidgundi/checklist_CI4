...
class ServicesMaster extends ResourceController
{
     ...
     public function getServicesListForAdmin($month)
     {
          ...
          // Connect to secondary DB once
          $db2 = \Config\Database::connect('secondary');
          $db2Name = $db2->database;
          // Define table references for cross-database joins
          $branchesTable = "$db2Name.branches";
          $usersTable    = "$db2Name.new_emp_master";
          ...
          if (!empty($selectedCluster) && $selectedCluster > 0) {
               $builder->where('services.cluster_id', $selectedCluster);
          }
          if (!empty($selectedBranch) && $selectedBranch > 0) {
               $builder->where('services.branch_id', $selectedBranch);
          }
          ...
          // 🧩 Collect all service IDs to fetch files in one go
          ...
          // Attach files to their corresponding services
