...
class BM_TasksModel extends Model
{
     ...
     //     return !empty($taskResults) ? $taskResults : [];
     // }
     public function getBranchComboTaskListNew($role, $user, $selectedMonth, $selectedBranch, $selectedCluster, $selectedZone, $selectedDate, $selectedToDate)
     {
          ...
          $builder = $this->db->table('bm_tasks m')
               ->select('m.*, m.taskDate as mtaskDate, ')
               ->join('user_map um', 'FIND_IN_SET(m.branch, um.branches)', 'left');
          // Apply month filter
          if (!empty($selectedMonth)) {
               $builder->where('DATE_FORMAT(m.taskDate, "%Y-%m")', $selectedMonth);
          }
          // Apply user's branch restrictions if not SUPER_ADMIN
          if ($role != 'SUPER_ADMIN' && !empty($branchIds)) {
               $builder->whereIn('m.branch', $branchIds);
          } elseif ($role != 'SUPER_ADMIN' && empty($branchIds)) {
               // If user has no branches mapped and is not SUPER_ADMIN, return empty result
          ...
          if (!empty($selectedDate) && !empty($selectedToDate)) {
               ...
               // If no date filters are provided, show last 30 days data
               $today = date('Y-m-d');
               $tenDaysAgo = date('Y-m-d', strtotime('-10 days'));
               $builder->where('m.taskDate >=', $tenDaysAgo);
               $builder->where('m.taskDate <=', $today);
               //   log_message('info', 'No date filter provided, showing last 30 days data from ' . $tenDaysAgo . ' to ' . $today);
          }
          $builder->orderBy('m.taskDate', 'desc');
          $results = $builder->get()->getResultArray();
          if (empty($results)) {
               return ['status' => false, 'message' => 'No task data found.'];
          }
          ...
          foreach ($results as &$task) {
               ...
               // Find cluster info
               $clusterInfo = $this->db->table('clusters')
                    ->select('cluster_id, cluster')
                    ->where("FIND_IN_SET('" . $task['branch'] . "', branches)")
                    ->get()
                    ->getRowArray();
               $task['cluster_id'] = $clusterInfo['cluster_id'] ?? '';
               $task['cluster'] = $clusterInfo['cluster'] ?? '';
               // Find zone info
               $zoneInfo = $this->db->table('zones')
                    ->select('z_id, zone')
                    ->where("FIND_IN_SET('" . $task['branch'] . "', branches)")
                    ->get()
                    ->getRowArray();
          ...
          $uniqueResults = array_values(array_reduce($results, function ($carry, $item) {
               $carry[$item['mid']] = $item;
               return $carry;
          }, []));
     ...
     //         $clusterIds = explode(',', $selectedCluster);
     //         $builder->whereIn('um.cluster', $clusterIds);
     //     }
     //     if (!empty($selectedBranch) && $selectedBranch != '0') {
     //         $builder->where('m.branch', $selectedBranch);
     //     }
     ...
     //         $builder->where('m.taskDate', $selectedDate);
     //     } else {
     //         // If no date filters are provided, show last 4 days data
     //         $today = date('Y-m-d');
     //         $fourDaysAgo = date('Y-m-d', strtotime('-4 days')); // -3 days + today = 4 days total
     //         $builder->where('m.taskDate >=', $fourDaysAgo);
     //         $builder->where('m.taskDate <=', $today);
     //         log_message('info', 'No date filter provided, showing last 4 days data from ' . $fourDaysAgo . ' to ' . $today);
     //     }
     //     $builder->orderBy('m.taskDate', 'desc');
     //     $results = $builder->get()->getResultArray();
     //     // Enhance results with additional data
     ...
     //         // Add branch name
     //         $task['branch_name'] = $branchMap[$task['branch']] ?? '';
     //         // Find cluster info where branch is in the comma-separated branches list
     //         $clusterInfo = $this->db->table('clusters')
     //             ->select('cluster_id, cluster')
     //             ->where("FIND_IN_SET('" . $task['branch'] . "', branches)")
     //             ->get()
     ...
     //         $task['cluster_id'] = $clusterInfo['cluster_id'] ?? '';
     //         $task['cluster'] = $clusterInfo['cluster'] ?? '';
     //         // find zone info where branch is in the comma-separated branches list
     //         $zoneInfo = $this->db->table('zones')
     //             ->select('z_id, zone')
     //             ->where("FIND_IN_SET('" . $task['branch'] . "', branches)")
     //             ->get()
     ...
     public function getBM_TaskListForAdmin($role, $user, $selectedMonth, $selectedBranch, $selectedCluster, $selectedZone, $selectedDate, $selectedToDate, $searchText)
     {
          ...
          if (!empty($selectedDate) && !empty($selectedToDate)) {
               ...
               $builder->where('DATE(m.taskDate) <=', date('Y-m-d', strtotime($selectedToDate)));
          } elseif (!empty($selectedDate)) {
               $builder->where('DATE(m.taskDate)', date('Y-m-d', strtotime($selectedDate)));
          } else {
               $today = date('Y-m-d');
          ...
          if (!empty($searchText)) {
               $results = array_filter($results, function ($task) use ($searchText) {
                    return stripos($task['taskDate'], $searchText) !== false ||
                         stripos($task['branch_name'], $searchText) !== false ||
                         stripos($task['created_by'], $searchText) !== false ||
                         stripos($task['cluster'], $searchText) !== false ||
                         stripos($task['zone'], $searchText) !== false ||
                         stripos($task['fname'], $searchText) !== false ||
     ...
     //      // Fetch branch data using the refactored method
     //      $branchData = $this->getBranchData();
     //      // Create lookup maps
     //      $empData = $db2->table('new_emp_master')->select('emp_code, fname, lname')->get()->getResultArray();
     //      $empMap = [];
     ...
     //           $branchMap[$branch['id']] = $branch['SysField'];
     //      }
     //      // Main query builder for bm_tasks
     //      $builder = $this->db->table('bm_tasks m')
     //           ->select('m.*, m.taskDate as mtaskDate')
     //           ->join('user_map um', 'FIND_IN_SET(m.branch, um.branches)', 'left');
     //      // Apply filters
     ...
     //           $task['zone'] = $zoneInfo['zone'] ?? '';
     //      }
     //      // Apply searchText filter to the final output
     //      if (!empty($searchText)) {
     //           $results = array_filter($results, function ($task) use ($searchText) {
     //                return stripos($task['taskDate'], $searchText) !== false ||
     //                     stripos($task['branch_name'], $searchText) !== false ||
