...
class Profile extends BaseController
{
     ...
     public function index()
     {
          ...
          // Ensure `getUserProfiles()` returns an array or handle `$user` as an object
          $user = $this->getUserProfiles($bmid);
          ...
          if (!empty($user['mobile']) && empty($user['audited_by_mobile'])) {
               $user['audited_by_mobile'] = $user['mobile'];
          }
     ...
     public function getUserProfiles($bmid)
     {
          ...
          return $profileModel->where('emp_code', $bmid)->first();
     }
}
