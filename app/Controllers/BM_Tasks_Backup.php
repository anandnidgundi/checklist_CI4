...
class BM_Tasks extends BaseController
{
     ...
     public function addBM_Task()
     {
          ...
          $data = [
               ...
               'mt1200' => $jsonData->mt1200 ?? null,
               'mt1201' => $jsonData->mt1201 ?? null,
               'mt1202' => $jsonData->mt1202 ?? null,
               'branch' => $branch,
               'emp_code' => $emp_code,
          ...
          if ($branch > 0) {
               ...
               if ($existedRecord) {
                    return $this->respond(['status' => true, 'message' => 'Record already exists for this date and branch.', 'data' => $existedRecord], 200);
               }
     ...
     public function getBranchComboTaskListNew()
     {
          ...
          // Pass the parameters to the model 
          // Pass the parameters to the model
          $mtDetails = $bmTasksModel->getBranchComboTaskListNew($role, $user, $selectedMonth, $selectedBranch, $selectedCluster, $selectedZone, $selectedDate, $selectedToDate);
          if ($mtDetails) {
               return $this->respond(['status' => true, 'message' => 'BM Task Details.', 'data' => $mtDetails], 200);
          } else {
               return $this->respond(['status' => false, 'message' => 'BM Task Details not found'], 404);
     ...
     public function getBM_TaskListForAdmin()
     {
          ...
          $mtDetails = $bmTasksModel->getBM_TaskListForAdmin(
               ...
               $user,
               $selectedMonth,
               $selectedBranch,  // ← Pass the actual branch_id or '0'
               $selectedCluster,
               $selectedZone,
               $selectedDate,
               $selectedToDate,
          ...
          if ($mtDetails) {
               return $this->respond([
                    'status' => true,
                    'message' => 'BM Task Details.',
                    'data' => $mtDetails
               ], 200);
          } else {
               return $this->respond([
                    'status' => false,
                    'message' => 'BM Task Details not found'
               ], 404);
          }
     }
     ...
     //      // Get clusters with branches for the user
     //      $clusters = $deptModel->getClustersWithBranches($user);
     //      // Extract all branch IDs from the cluster list
     //      $allowedBranchIds = [];
     ...
     //      $selectedDate = (!empty($requestData->selectedDate)) ? $requestData->selectedDate : '';
     //      $selectedToDate = (!empty($requestData->selectedToDate)) ? $requestData->selectedToDate : '';
     //      // Apply final branch filter: either selected branch (if provided) or allowed branch IDs
